  ...
  <div class="white-bg">
    <Form :label-width="150" class="search-form" ref="formValidate" :model="formValidate" :rules="ruleValidate">
      ...
      <Row type="flex" justify="space-between">
        <Col :sm="8" :xs="24">
        <FormItem label="入库单号:">{{inCode}}</FormItem>
        </Col>
        <Col :sm="8" :xs="24">
    ...
    <Row>
      ...
      <div class="pull-left table-title">
        ...
        <Button class="margin-left-10" type="default" @click="download" size="small">下载模板</Button>
      </div>
        ...
        <table cellspacing="0" cellpadding="0" border="0" width="100%">
          ...
          <tbody>
            <tr v-for="(trItem,index) in tableData" :key="index" @keyup="keyCopyFun($event,index)">
              ...
              <td>
                <div class="ivu-table-cell">
                  <Select v-model="tableData[index].brandCode.value" filterable remote :remote-method="remoteFactory" @on-change="changeBrand($event,index)" label-in-value :class="tableData[index].brandCode.status || tableData[index].brandCode.value ? '' : 'error'">
                    <Option v-for="(item,index) in factoryTbList" :value="item.brandCode" :key="index">{{item.brandName}}</Option>
                  </Select>
                </div>
              ...
              <td>
                <div class="ivu-table-cell">
                  <Select label-in-value filterable :class="tableData[index].breedCode.status || tableData[index].breedCode.value ? '' : 'error'" v-model="tableData[index].breedCode.value" @on-change="changeBreed($event,index)">
                    <Option :value="tableData[index].breedCode.value">{{ tableData[index].breedName }}</Option>
                  </Select>
                </div>
              ...
              <td>
                <div class="ivu-table-cell">
                  <InputNumber :class="tableData[index].number.status || tableData[index].number.value ? '' : 'error'" style="width:100%;" :maxlength="10" :min="1" v-model="tableData[index].number.value" @on-change="countTonage(index)" />
                </div>
              </td>
              <td>
                <div class="ivu-table-cell input-number-no-operate">
                  <InputNumber :class="tableData[index].perAmount.status ? '' : 'error'" style="width:100%;" :maxlength="20" :min="0" v-model="tableData[index].perAmount.value" @on-change="countTonage(index,$event)" />
                  <div v-if="!tableData[index].perAmount.status && tableData[index].perAmount.value" class="error-tip">最多4位小数！</div>
                </div>
              </td>
              <td>
                <div class="ivu-table-cell">
...
export default {
  ...
  methods: {
    download(){
      window.open('/pwms/inventory/in/downInEnterTemplates?templeteName=inEnterTemplates.xlsx');
    },
    remoteFactory(query,index){
      console.log(query)
      if (query !== '') {
                    setTimeout(() => {
                        this.factoryTbList = this.factoryList.filter(item => item.brandName.toLowerCase().indexOf(query.toLowerCase()) > -1);
                    }, 200);
                } else {
                    this.factoryTbList = [];
                }
    ...
    getInfo() {
      InventoryInEnterInfo({
        ...
        if(res.data.code == "200"){
            ...
            let inEnterItems = data && data.inEnterItems ? data.inEnterItems : [];
            let inEnterList = data && data.inEnterList ? data.inEnterList : {};
            this.inCode = inEnterList.inCode;
            this.formValidate = {
              ownerId:inEnterList.ownerId ? inEnterList.ownerId.toString() : "",
              ownerName:inEnterList.ownerName,
              ownerContract: inEnterList.ownerContract,
              ownerPhone: inEnterList.ownerPhone,
              ownerNoticeCode: inEnterList.ownerNoticeCode,
              inTime: inEnterList.inTime ? new Date(inEnterList.inTime) : "",
              carContract: inEnterList.carContract,
              carNumber: inEnterList.carNumber,
              carPhone: inEnterList.carPhone,
              receiveId: inEnterList.receiveId ? inEnterList.receiveId.toString() : "",
              receiveName:inEnterList.receiveName,
              remark: inEnterList.remark
            }
            let cranemanIds = data && data.cranemanWorkers ? data.cranemanWorkers : [];
            this.cranemanIds = cranemanIds.map((item) => {
              return item.employeeId.toString()
            })
            let stevedoreIds = data && data.stevedoreWorkers ? data.stevedoreWorkers : [];
            this.stevedoreIds = stevedoreIds.map((item) => {
              return item.employeeId.toString()
            })
            let data1 = inEnterItems.map((item, index) => {
              ...
              return {
                code: { status: true, value: item.code },
                ...
                specName: { isUpload:true,status: true, value: item.specName },
                materialCode: { isUpload:true,status: true, value: item.materialCode },
                materialName: item.materialName,
                type:{ isUpload:true,status: true, value: item.type.toString() },
                inventoryType: { isUpload:true,status: true, value: item.inventoryType.toString() },
                storageAreaId: { isUpload:true,status: true, value: item.storageAreaId.toString() },
                storageArea:item.storageArea,
                storagePlaceId: { isUpload:true,status: true, value: item.storagePlaceId.toString() },
                storagePlace:item.storagePlace,
                number: { isUpload:true,status: true, value: item.number },
                perAmount: { isUpload:true,status: perAmountStatus, value: item.perAmount },
                totalAmount: { isUpload:true,status: true, value: totalAmount },
                weightType: { isUpload:true,status: true, value: item.weightType.toString() },
                baleNumber: { isUpload:true,status: true, value: item.baleNumber },
                gyCode:item.gyCode,
                inEnterItemId:item.id,
          ...
          if (!trError) {
            ...
            for(var i=0;i<this.tableData.length;i++){
              ...
              inEnterItemImportDTOs.push({
                inEnterItemId:item.inEnterItemId ? item.inEnterItemId : 0,
            ...
            InventoryInUpdateInEnterList({
              ...
              receiveId: parseInt(this.formValidate.receiveId),
        ...
        let data1 = data.data.itemDTOs.map((item, index) => {
         ...
         // this.breedChangeSave(item.breedCode,index);
          ...
          return {
            ...
            weightType: { status: true, value: item.weightType.toString() },
