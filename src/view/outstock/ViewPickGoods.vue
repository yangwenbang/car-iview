<template>
    <div class="white-bg view-pick-goods">
        <div class="table-scrollable left-right-table">
            <table border="1">
                <tbody>
                    ...
                    <tr v-if="settleTimeType==1">
                        <td class="left-bg">结算客户：</td>
                        <td>{{form.settleCustomerName}}</td>
                        <td class="left-bg">结算方式：</td>
                        <td>{{form.settleType==0?'月结':(form.settleType==1?'现结':'其他')}}</td>
                    </tr>
                    <tr>
        ...
        <Row class="margin-top-20 margin-bottom-10">
            ...
            <div class="pull-right count-content">
                <span class="margin-right-10">提货总数量：
                    <b class="tip">{{form.deliveryNumber}}</b> 件</span>
                <span class="margin-right-10">提货总重量：
                    <b class="tip">{{form.deliveryWeight}}</b> 吨</span>
            </div>
            </Col>
        </Row>
        ...
        <div class="text-center margin-top-20">
            <Button type="primary" class="btn-common-width" @click="printFun">打印</Button>
        </div>
    </div>
</template>
<script>
import { closeNowTag,isAcrobatPluginInstall, formatWeight,formatPrice } from "@/libs/util";
import { queryDeliveryDetail } from "@/api/outstock";
export default {
  name: "ViewPickGoods",
  data() {
    return {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          minWidth: 100,
          render: (h, params) => {
            return h("span", {}, params.row.perAmount ? Number(params.row.perAmount).toFixed(4) : 0);
          }
        },
        {
          title: "提货总重量(吨)",
          ...
          minWidth: 120,
          render: (h, params) => {
            return h("span", {}, params.row.weight ? Number(params.row.weight).toFixed(4) : 0);
          }
        },
        {
            title: "过磅重量(吨)",
  ...
  methods: {
    printFun(){
       ...
       if(result){
          window.open('/api/pwms/inventory/delivery/printDeliveryOrder?id='+this.$route.query.id)
       }
    },
    getDetail() {
      queryDeliveryDetail({
...
<style scoped>
.view-pick-goods >>> .ivu-table-footer{
	height: 0px !important;
	line-height: 0px !important;
}
.view-pick-goods  .other-price-info{
	height: 48px ;
	line-height: 48px ;
}
</style>
