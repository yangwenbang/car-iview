<template>
  <div class="white-bg">
    <Form :label-width="150" class="search-form" ref="formParams" :model="formParams" :rules="ruleValidate">
      <Row>
        <Col :sm="8" :xs="24">
        ...
        <Col :sm="8" :xs="24">
        <FormItem label="货物类型:">
          <div class="val-text">{{formParams.type==0?'原料':formParams.type==1?'成品':formParams.type==2?'废料':''}}</div>
        </FormItem>
        </Col>
        <Col :sm="8" :xs="24">
        <FormItem label="结算方式:">
          <Select v-model="formParams.settleType"  :disabled="disabledSettleType">
        ...
        <FormItem label="结算客户:" prop="settleCustomerId">
          <Select v-model="formParams.settleCustomerId" label-in-value filterable clearable @on-change="changeCustomer">
        ...
        <Col :sm="8" :xs="24">
        <FormItem label="结算时间:">
          <RadioGroup v-model="settleTimeType">
    ...
    <Table :row-class-name="rowClassName" :columns="tableColumns" :data="tableData" border class="validTable vertical-middle table-nowrap">
      ...
      <template slot-scope="{ row, index }" slot="outSinglePrice" v-if="settleTimeType==0">
        <div v-if="row.packId">
          <div v-for="(item,index1) in row.packList" :key="index1" :class="index1 > 0 ? 'table-line table-mn' : 'table-mn'">
      ...
      <template slot="otherPrice" slot-scope="{ row, index }" v-if="settleTimeType==0">
        <div v-if="row.packId">
          <div v-for="(item,index1) in row.packList" :key="index1" :class="index1 > 0 ? 'table-line table-mn' : 'table-mn'">
            <Form :ref="'formOtherPrice'+index + '-' + index1" :model="item">
              <FormItem class="input-number-no-operate" prop="otherPrice">
      ...
      <div slot="footer" class="table-foot">
        <Form :label-width="150" class="search-form margin-top-15" ref="formPrice" :model="formPrice" v-if="settleTimeType==0">
          <Row>
            <Col :sm="8" :xs="24">
    ...
    <out-goods ref="modalChild" :showModal="showModal" :ownerName="formParams.ownerName" :ownerId="ownerId" :type="formParams.type" :selectedIds="selectedIds" @hide-modal="hideModel"></out-goods>
...
export default {
          ...
          render: (h, data) => {
            if (data.row.packId) {
              ...
              data.row.packList.forEach((item, index) => {
                ...
                arr.push(
                  h("div", { class: line }, formatWeight(item.perAmount))
                );
              });
              return h("div", arr);
            } else {
          ...
          render: (h, data) => {
            if (data.row.packId) {
              ...
              data.row.packList.forEach((item, index) => {
                ...
                arr.push(
                  h("div", { class: line }, formatWeight(item.totalWeight))
                );
              });
              return h("div", arr);
            } else {
          ...
          render: (h, data) => {
            if (data.row.packId) {
              ...
              data.row.packList.forEach((item, index) => {
                ...
                arr.push(
                  h("div", { class: line }, formatPrice(item.outTotalPrice))
                );
              });
              return h("div", arr);
            } else {
          ...
          render: (h, data) => {
            if (data.row.packId) {
              ...
              data.row.packList.forEach((item, index) => {
                ...
                arr.push(
                  h("div", { class: line }, formatPrice(item.totalMoney))
                );
              });
              return h("div", arr);
            } else {
  ...
  methods: {
    ...
    changeCustomer(option) {
      if (option) {
        this.formParams.settleCustomerName = option.label;
      } else {
        this.formParams.settleCustomerName = "";
      }
      this.getIsMonthSettle()
    },
    //获取用户结算信息
    getIsMonthSettle() {
      if(this.formParams.settleCustomerId) {
        this.ownnerList.map((o) => {
          if(this.formParams.settleCustomerId == o.id) {
            this.formParams.settleType = 1;
      ...
      if (data) {
        ...
        for (let i = 0; i < data.length; i++) {
          if(data[i].packId && packIds.indexOf(data[i].packId) >= 0){
            ...
            data[i].outSinglePrice = data[i].unitPrice
              ? String(data[i].unitPrice)
              : "";

            //出库费用(元) = 出库总重量*出库单价
            data[i].outTotalPrice = data[i].totalWeight * data[i].outSinglePrice;
            ...
            this.tableData.push({
              ...
              realOutNumber: data[i].realOutNumber
                ? parseFloat(data[i].realOutNumber)
                : null,
              perAmount: data[i].perAmount ? parseFloat(data[i].perAmount) : null,
              totalWeight: data[i].totalWeight,
              weightType: data[i].weightType,
              outSinglePrice: data[i].outSinglePrice
                ? parseFloat(data[i].outSinglePrice)
                : null,
              outTotalPrice: data[i].outTotalPrice
                ? parseFloat(data[i].outTotalPrice)
                : null,
              otherPrice: data[i].otherPrice
                ? parseFloat(data[i].otherPrice)
                : null,
              totalMoney: data[i].totalMoney
                ? parseFloat(data[i].totalMoney)
                : null,
              wrap: data[i].wrap,
    ...
    getList() {
      OutSettleDetail({
        ...
        if (res.data.code == "200") {
          ...
          this.formParams.settleCustomerId = this.formParams.settleCustomerId
            ? this.formParams.settleCustomerId.toString()
            : "";
          this.formPrice.extraCharge =
            data.extraCharge || data.extraCharge == 0
              ? parseFloat(data.extraCharge)
              : null;
          this.formPrice.packageCharge =
            data.packageCharge || data.packageCharge == 0
              ? parseFloat(data.packageCharge)
              : null;
          ...
          for (let i = 0; i < data.itemList.length; i++) {
            if (data.itemList[i].packId) {
              let count = -1;
              for (let j = 0; j < tableArr.length; j++) {
                if (
                  tableArr[j].packId == data.itemList[i].packId &&
                  tableArr[j].id != data.itemList[i].id
                ) {
                  count = j;
                }
              }
              if (count >= 0) {
                tableArr[count].packList.push({
                  ...
                  remainNum: Number(data.itemList[i].remainNum),
                  realOutNumber: data.itemList[i].number
                    ? parseFloat(data.itemList[i].number)
                    : null,
                  perAmount: data.itemList[i].perAmount
                    ? parseFloat(data.itemList[i].perAmount)
                    : null,
                  totalWeight: data.itemList[i].weight
                    ? parseFloat(data.itemList[i].weight)
                    : null,
                  weightType: data.itemList[i].weightType,
                  outSinglePrice: data.itemList[i].price
                    ? parseFloat(data.itemList[i].price)
                    : null,
                  outTotalPrice: data.itemList[i].outAmount
                    ? parseFloat(data.itemList[i].outAmount)
                    : null,
                  otherPrice: data.itemList[i].otherCharge
                    ? parseFloat(data.itemList[i].otherCharge)
                    : null,
                  totalMoney: data.itemList[i].amount
                    ? parseFloat(data.itemList[i].amount)
                    : null,
        ...
        for (let i = 0, len = this.tableData.length; i < len; i++) {
          if (!this.tableData[i].isDisabled) {
            if (this.tableData[i].packId) {
              var newValid = true;
              for (var j = 0; j < this.tableData[i].packList.length; j++) {
                this.$refs["formSinglePrice" + i + "-" + j][0].validate(
                  valid => {
                    if (!valid) {
                      newValid = false;
                    }
    ...
    handleSubmit() {
      ...
      if (tableFlag) {
        ...
        this.tableData.forEach((item, index) => {
          if (item.packId && !item.isDisabled) {
        ...
        if (tbLength > 0) {
          ...
          if (this.settleTimeType == 0) {
            para.extraCharge = this.formPrice.extraCharge
              ? parseFloat(this.formPrice.extraCharge).toFixed(2)
              : 0;
            para.packageCharge = this.formPrice.packageCharge
              ? parseFloat(this.formPrice.packageCharge).toFixed(2)
              : 0;
          }
              ...
              if (this.settleTimeType == 0) {
                outSettleItemDTOs.push({
                  weight: submitData[i].totalWeight
                    ...
                    : 0,
                  id: submitData[i].id ? submitData[i].id : "",
