 ...
 <div class="white-bg">
    <Form :label-width="150" class="search-form" :rules="ruleValidate" ref="formValidate" :model="formValidate">
      <Row type="flex" justify="space-between">
        <Col :sm="8" :xs="24" >
          <FormItem label="用户名:" prop="userName">
            <Input v-model="formValidate.userName" :maxlength="20" disabled></Input>
          </FormItem>
        </Col>
        <Col :sm="8" :xs="24" >
          <FormItem label="姓名:" prop="nickName">
            <Input v-model="formValidate.nickName" :maxlength="20" disabled></Input>
          </FormItem>
        </Col>
        <Col :sm="8" :xs="24">
     ...
     <div class="text-center margin-top-10">
      <Button type="primary" class="btn-common-width"  @click="handleSubmit('formValidate')" :disabled="submitDisabled">确定修改</Button>
    </div>
  </div>
</template>
<script>
import { showAccount,getWarehouse,queryRoleList,updateUser} from '@/api/account'
...
            }
      ...
      promiseAll(){
          let promiseArray=[];
          promiseArray.push(showAccount({userId:this.formValidate.userId}));
          promiseArray.push(queryRoleList(
            { 
               "status":0,
               "platform":0,
               "pageQuery":0}
          ));
          promiseArray.push(getWarehouse());
          Promise.all(promiseArray).then(res=>{
            if(res[0].data.code == 200){
                this.formValidate.userName =  res[0].data.data.userName;
                this.formValidate.nickName = res[0].data.data.nickName;
                this.formValidate.mobile = res[0].data.data.mobile;
                this.formValidate.remark = res[0].data.data.remark;
                this.formValidate.password = res[0].data.data.password;
                this.formValidate.keyword = res[0].data.data.password;
                this.formValidate.card = res[0].data.data.card;
                this.formValidate.type = res[0].data.data.type;
                this.formValidate.userWarehouseDTOS = res[0].data.data.warehouseId ? res[0].data.data.warehouseId.split(",") : [];
                let tempUserRoleName = res[0].data.data.userRoleName ? res[0].data.data.userRoleName.split(","):[]
                let tempUserRoleId = res[0].data.data.userRoleId ? res[0].data.data.userRoleId.split(","):[]
                this.currentSelect = tempUserRoleName.map((item,index)=>{
                  return {
                    roleId:tempUserRoleId[index],
                    roleName:item
                  }
                })
            }
            if(res[1].data.code == 200 ){
              this.tableData = res[1].data.data.recordList
              this.currentSelect.forEach((item,index)=>{
                res[1].data.data.recordList.forEach((iitem,iindex)=>{
                  if(item.roleId == iitem.id){
            ...
            if(res[2].data.code == 200 ){
                this.formValidate.warehouseList = res[2].data.data.map(o => {
                  return {
                    warehouseName: o.warehouseName,
                    warehouseId: o.id.toString()
                  }
                })
                 this.formValidate.userWarehouseDTOS.map(a => {
