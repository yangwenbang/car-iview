        ...
        <Form class="submit-form" ref="formValidate" :model="formValidate" :rules="ruleValidate" label-position="right" :label-width="130">
            <Row :gutter="10">
                ...
                <FormItem label="期望交货日期:" prop="expectDeliverTime">
                    <DatePicker type="date" v-model="formValidate.expectDeliverTime" placeholder="请选择" style="width: 100%"></DatePicker>
                </FormItem>
                </Col>
                <Col :lg="8" :xs="24">
                ...
                <FormItem label="加工类别:" prop="processType">
                    <RadioGroup v-model="formValidate.processType" @on-change="switchType" >
                        <Radio :label="0">开平</Radio>
                        <Radio :label="1">纵剪</Radio>
                    </RadioGroup>
                </FormItem>
                </Col>
                <Col :lg="8" :xs="24">
            ...
            <Tabs>
                <TabPane label="开平加工单" v-if="tab">
                    <NewProcessOpen @dataChange="changeTableData" @removeTableData="deleteRow" @showModal="showCoilModal" :isEdit="false" :isValidated="validated" :propData="submitSelect" :owner="formValidate.ownerId?formValidate.ownerId:0"></NewProcessOpen>
                </TabPane>
                <TabPane label="纵剪加工单" v-else>
                    <NewProcessBillScissors @dataChange="changeTableData" @removeTableData="deleteRow" @showModal="showCoilModal" :isEdit="false" :isValidated="validated" :propData="submitSelect" :owner="formValidate.ownerId?formValidate.ownerId:0"></NewProcessBillScissors>
                </TabPane>
            </Tabs>
            ...
            <Row type="flex" justify="space-between">
                ...
                <FormItem label="货物类型:">
                    <Select v-model="modalParams.type" clearable placeholder="请选择货物类型" disabled>
                        <Option v-for="(item, index) in goodsList" :value="item.value" :key="index">{{ item.label }}</Option>
                    </Select>
                </FormItem>
                </Col>
                <Col :sm="8" :xs="24">
...
import { InventoryProcessSaveProcess, InventoryProcessCustomerList, InventoryQueryVInventoryItem } from '@/api/process';
export default {
    name: "NewProcessBill",
    components: {
    ...
    data() {
        return {
            ...
            formValidate: {
                ...
                mobile: '',
                processType: 0,
                isIntegrate: this.$store.state.user.warehouse.processDefaultSettleTime,
                settleType: '',
                processDemand: ''
            },
            modalTitle: "选择原料",
    ...
    methods: {
        ...
        changeTableData(data) {
            ...
            this.groupRadio = JSON.parse(JSON.stringify(data.radio));
        },
        handleSubmit(name) {
            this.validated = true;
        ...
        showCoilModal(data) {
            ...
            this.modalTitle = data==0?'选择原料':'选择成品';
            this.search();
            this.diabledChecked();
            this.showModal = true;
        },
        ...
        getIsMonthSettle() {
            if(this.formValidate.ownerId) {
                this.ownerNameList.map((o) => {
                    if(this.formValidate.ownerId == o.customerId) {
