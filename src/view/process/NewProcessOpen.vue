          ...
          >
            <thead>
              ...
              <tr>
                ...
                <th>
                  <div class="ivu-table-cell"><span class="color-red">*</span>加工长度(mm)</div>
                </th>
                <th>
                  <div class="ivu-table-cell"><span class="color-red">*</span>数量(张)</div>
                </th>
                <th class="text-right">
                  <div class="ivu-table-cell">理重(吨)</div>
                </th>
                <th class="text-center">
                  <div class="ivu-table-cell">余开/全开
                  </div>
                </th>
                <th>
                ...
                <th class="text-right">
                  <div class="ivu-table-cell">已用长度(m)</div>
                </th>
                <th class="text-right">
                  <div class="ivu-table-cell">剩余长度(m)</div>
                </th>
                <th class="text-right">
              ...
              <template v-for="(data, index) in tableData1">
                <tr :key="index" :class="{'tr-even': index%2 == 1}">
                  ...
                  <td>
                    <div class="ivu-table-cell">
                      <Input
                        @on-change="computedAll"
                        v-model="data.processProducts[0].specWidth"
                        :maxlength="20"
                        :class="{'error-border': validate==true && !validateColumns1(data.processProducts[0].key,0,'specWidth').flag}"
                      ></Input>
                  ...
                  <td class="text-center">
                    <div class="ivu-table-cell">
                      <template v-if="data.weight == data.processWeight">
                        <Button
                          type="info"
                          size="small"
                          @click="openSurplus(data.processProducts[0])"
                          v-if="data.processProducts.length == 1"
                        >全开</Button>
                      </template>
                  ...
                  >
                    <td>
                      <div class="ivu-table-cell">
                        <Input
                          @on-change="computedAll"
                          v-model="process.specWidth"
                          :maxlength="20"
                          :class="{'error-border': validate==true && !validateColumns1(process.key,i,'specWidth').flag}"
                        ></Input>
                    ...
                    <td class="text-center">
                      <div class="ivu-table-cell">
                        <template v-if="tableData1[getCurentRowIndex(process.key)].weight == tableData1[getCurentRowIndex(process.key)].processWeight">
                          <Button
                            type="info"
                            size="small"
                            @click="openSurplus(process)"
                            v-if="i == tableData1[getCurentRowIndex(process.key)].processProducts.length -1"
                          >余开</Button>
                        </template>
                        <template v-else>-</template>
                      </div>
...
export default {
  ...
  methods: {
    ...
    // 余开，全开
    openSurplus(data) {
      let index = this.getCurentRowIndex(data.key);
      let sumColumns3 = 0;
      let rowData = this.tableData1[index];
      let processProducts = rowData.processProducts;
      //数量=（剩余用料kg）/（厚度mm*宽度m*7.85*加工长度m）
      for(let i = 0; i <  processProducts.length -1; i++) {
        let temp = processProducts[i];
        sumColumns3 += Number(temp.weight);
      }
      //剩余用料kg
      let surplusWeigth = (Number(rowData.processWeight) - sumColumns3) * 1000;
      //加工长度m
      let machineLong = Number(processProducts[processProducts.length - 1].specWidth)/1000;
      if(machineLong > 0) {
        this.tableData1[index].processProducts[processProducts.length - 1].number = Number((surplusWeigth / ((Number(rowData.col20)/1000)*Number(rowData.col21)*7.85*machineLong)).toFixed(0));
        this.computedAll();
      }
    },
    //合计
    ...
    computedAll() {
      ...
      tempArr.map((o, i) => {
        ...
        o.processProducts.map((o1, j) => {
            ...
            //理重
            this.tableData1[i].processProducts[j].weight = formatWeight((this.isNaN(o.col20) * (this.isNaN(o.col21)/1000) *  (this.isNaN(o1.specWidth)/1000) * this.isNaN(o1.number) * 7.85)/1000);
            //金额 = 加工单价*理重+其他费用
            ...
            //处理余开/全开
            if(o.weight == o.processWeight) {
              if(o.processProducts.length == 1) {
                //全开
                this.tableData1[i].processProducts[j].isRemainOpen = 0;
              }else if(o.processProducts.length > 1){
                if(j == o.processProducts.length - 1) {
                  //余开
                  this.tableData1[i].processProducts[j].isRemainOpen = 1;
                } else {
                  //其他
                  this.tableData1[i].processProducts[j].isRemainOpen = 2;
                }
        ...
        //已用米数(m)=已填写的加工规格*数量
        this.tableData1[i].usedWeidth = formatPrice(sumCol7/1000);
        //剩余米数（m）=原卷的长度-已用的加工规格*数量
        //默认m=用料kg/（厚度mm*宽度m*7.85）
        let defaultLong = formatPrice((this.isNaN(o.processWeight) * 1000)/(this.isNaN(o.col21) * (this.isNaN(o.col20)/1000) * 7.85));
        this.tableData1[i].remainWidth = formatPrice(defaultLong - this.tableData1[i].usedWeidth);
        //金额 = 加工单价*用料+其他费用
      ...
      this.currentSelect.map((o, i) => {
        ...
        if (!isHave) {
          ...
          this.tableData1.push({
            ...
            //金额 = 加工单价*用料+其他费用
            totalPrice: this.edit?formatPrice(o.totalPrice):0,
            //已用米数(m)=已填写的加工规格*数量
            usedWeidth: this.edit?formatPrice(o.usedWeidth):0,
    ...
    validateCol9(index, colName) {
      let rules = {
        flag: true,
        content: '',
      }
      let currentValue = this.tableData1[index][colName];
      if (!Number(currentValue)) {
        rules.flag = false;
        rules.content = '请输入大于0的数字';
      } else if(Number(currentValue) < 0) {
