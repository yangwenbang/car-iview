            ...
            <div class="table-content">
                <div class="ivu-table table1">
                    <table cellspacing="0" cellpadding="0" border="0" width="100%">
                        <thead>
                            <tr>
                                <th colspan="6" class="text-center"><div class="ivu-table-cell">母卷信息</div></th>
                                <th colspan="5" class="text-center"> <div class="ivu-table-cell">加工信息</div></th>
                                <th colspan="5" class="text-center"><div class="ivu-table-cell">实际加工数据录入</div></th>
                            ...
                            <tr>
                                ...
                                <th class="text-right"><div class="ivu-table-cell">加工长度(mm)</div></th>
                                <th class="text-right"><div class="ivu-table-cell">加工数量(张)</div></th>
                                <th class="text-right"><div class="ivu-table-cell">理重(吨)</div></th>
                                <th class="text-right"><div class="ivu-table-cell">单件理重(吨)</div></th>
                                <th class="text-right"><div class="ivu-table-cell">平均厚度(mm)</div></th>
                                <th><div class="ivu-table-cell">实际数量(张)</div></th>
                        ...
                        <tbody>
                            ...
                            <template v-for="(data, index) in tableData1">
                                <tr :key="data.materialId" :class="{'tr-even': index%2==1}">
                                    ...
                                    <td class="text-right" :rowspan="data.processProductsCopy.length>0?data.processProductsCopy.length:1"><div class="ivu-table-cell" v-if="data.processProductsCopy.length>0">{{data.averageThick|formatPrice}}</div></td>
                                    ...
                                    <td><div class="ivu-table-cell" v-if="data.processProductsCopy.length>0">
                                        <Select v-model="data.processProductsCopy[0].storageAreaId"  @on-change="changeSelect('storageAreaId',data.processProductsCopy[0].packCode, data.processProductsCopy[0].storageAreaId)" style="min-width:120px" :class="{'error-border':isValidate&&(data.processProductsCopy[0].storageAreaId==''||data.processProductsCopy[0].storageAreaId==undefined)}">
                                    ...
                                    <td><div class="ivu-table-cell" v-if="data.processProductsCopy.length>0">
                                        <Select v-model="data.processProductsCopy[0].storagePlaceId" @on-change="changeSelect('storagePlaceId',data.processProductsCopy[0].packCode, data.processProductsCopy[0].storagePlaceId)" style="min-width:120px" :class="{'error-border':isValidate&&(data.processProductsCopy[0].storagePlaceId==''||data.processProductsCopy[0].storagePlaceId==undefined)}">
                                ...
                                <template v-for="(process, i) in data.processProductsCopy">
                                    <tr :key="'efg' +index+ i" v-if="i > 0" :class="{'tr-even': getCurentRowIndex(process.key)%2==1}">
                                        ...
                                        <td><div class="ivu-table-cell">
                                            <Select v-model="process.storageAreaId" @on-change="changeSelect('storageAreaId', process.packCode, process.storageAreaId)" style="min-width:120px" :class="{'error-border':isValidate&&(process.storageAreaId==''||process.storageAreaId==undefined)}">
                                                <Option v-for="item in areaOptions" :value="item.id" :key="'g'+item.id">{{ item.name }}</Option>
                                            </Select>
                                            <div class="color-red" v-if="isValidate&&(process.storageAreaId==''||process.storageAreaId==undefined)">请选择库区</div>
                                            </div>
                                        </td>
                                        <td><div class="ivu-table-cell">
                                            <Select v-model="process.storagePlaceId" @on-change="changeSelect('storagePlaceId', process.packCode, process.storagePlaceId)" style="min-width:120px" :class="{'error-border':isValidate&&(process.storagePlaceId==''||process.storagePlaceId==undefined)}">
                                                <Option v-for="item in bitOptions" :value="item.id" :key="'h'+item.id">{{ item.name }}</Option>
                                            </Select>
                                            <div class="color-red" v-if="isValidate&&(process.storagePlaceId==''||process.storagePlaceId==undefined)">请选择库位</div>
                                            </div>
                                        </td>
                                        <td><div class="ivu-table-cell">{{process.packCode?process.packCode:'未打包'}}</div></td>
                                        <td>
                                            <div class="ivu-table-cell">
    ...
    methods: {
        ...
        computedThickWeight() {
            ...
            this.tableData1.map((o,i) => {
                ...
                let totalWidth = 0;
                //废料总重量
                let scrapTotalWeight = 0;
                o.processScrapList.map(o1 => {
                    if(o1.scrapType == 5) {
                        totalWidth += Number(o1.width) * o1.number;
                    }
                    scrapTotalWeight += Number(o1.weight);
                })
                //开平板长度总和
                let totalLong = 0;
                o.processProductsCopy.map((o1,j) => {
                    this.sumAmount += Number(o1.realNumber);
                    totalLong += Number(o1.specWidth) * Number(o1.realNumber);
                })
                let specNameArr = o.specName.split('*');
                let width = Number(specNameArr[1]);
                let thick = Number(specNameArr[0]);

                //平均厚度 = （用料总重量-废料总重量）/（开平板长度总和*（原卷宽度-边丝宽度总和）*7.85）
                this.tableData1[i].averageThick = (o.processWeight - scrapTotalWeight)*1000000000/(totalLong*(width - totalWidth)*7.85);
            })
        },
        ...
        formatData() {
            ...
            this.tableData1.map(o => {
                let totalLong = 0;
                o.processProductsCopy.map(o1 => {
                    totalLong += o1.realNumber*o1.specWidth;
                    let temp = {
                        id: o1.productId,
                        note: o1.note,
                ...
                o.processScrapList.map(o1 => {
                    o1.specName = o1.thick + "*" + o1.width + "*" + o1.long;
                     if(o1.scrapType == 5) {
                        //边丝
                       o1.weight = formatWeight((Number(o1.thick) * Number(o1.width) * Number(totalLong+totalLong1) * 7.85 * o1.number)/1000000000);
                    }
                })
                ...
                this.tableData1.map((o, i) => {
                    ...
                    o.processProducts.map(o1 => {
                        ...
                        if(o1.inventoryType == 2 || o1.inventoryType == 3 || o1.inventoryType == 0 || o1.inventoryType == 1) {
                            ...
                            o.processProductsCopy.push(JSON.parse(JSON.stringify(o1)))
                        }
                    })
                    ...
                    o.processProductsCopy.map((o1,j) => {
                        ...
                        this.sumAmount += Number(o1.realNumber);
                        totalLong += o1.realNumber*o1.specWidth;
                        //加工状态待录入数据时，解决后台未计算平均厚度、单件理重问题
                        if(this.status == 2) {
                            let realNumber = Number(o.number);
                            if(o1.realNumber !="" && o1.realNumber != 0) {
                                realNumber=Number(o1.realNumber);
                            }
                            // 单件理重 = 原卷宽度*原卷厚度*加工长度*7.85 
                            this.tableData1[i].processProductsCopy[j].perWeight = specNameArr[1] * specNameArr[0] * o1.specWidth * 7.85/1000000000;
                        }
                    })
